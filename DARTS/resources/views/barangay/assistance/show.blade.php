    ...
    <div class="space-y-8">
        ...
        <section class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            <div class="space-y-6">
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
                    <h2 class="text-2xl font-bold tracking-tight text-slate-900">Assistance Details</h2>

                    <div class="mt-6 grid gap-4 text-sm">
                        <div class="flex items-start justify-between gap-4">
                            <span class="text-slate-500">Case Summary</span>
                            <span class="max-w-[70%] text-right font-medium text-slate-900">{{ $assistanceDetail?->case_summary ?: '—' }}</span>
                        </div>

                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500">Requested Amount</span>
                            <span class="font-medium text-slate-900">
                                {{ $assistanceDetail?->requested_amount !== null ? number_format($assistanceDetail->requested_amount, 2) : '—' }}
                            </span>
                        </div>

                        <div class="flex items-start justify-between gap-4">
                            <span class="text-slate-500">Assessment Notes</span>
                            <span class="max-w-[70%] text-right font-medium text-slate-900">{{ $assistanceDetail?->assessment_notes ?: '—' }}</span>
                        </div>

                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500">Assessment Date</span>
                            <span class="font-medium text-slate-900">{{ $assistanceDetail?->assessment_date?->format('F d, Y') ?: '—' }}</span>
                        </div>

                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500">Claimant Name</span>
                            <span class="font-medium text-slate-900">{{ $assistanceDetail?->claimant_name ?: '—' }}</span>
                        </div>

                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500">Relationship to Beneficiary</span>
                            <span class="font-medium text-slate-900">{{ $assistanceDetail?->relationship_to_beneficiary ?: '—' }}</span>
                        </div>

                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500">Referral Destination</span>
                            <span class="font-medium text-slate-900">{{ $assistanceDetail?->referral_destination ?: '—' }}</span>
                        </div>
                    </div>
                </section>
